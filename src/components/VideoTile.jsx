...
export default function VideoTile({ src, poster, label, index = 0 }) {
  const videoRef = useInViewVideo({ threshold: 0.5 });
  ...
  return (
    ...
    >
      ...
      {label && (
        <p className="label absolute bottom-4 left-4 text-bone opacity-0 transition-opacity duration-500 group-hover:opacity-100">
          {label}
        </p>
      )}
    </motion.div>
